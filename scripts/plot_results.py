...
METRIC_COLS = {
    ...
    "avg_search_success_probes": (
        "avg_search_success_probes",
        "sem_search_success_probes",
    ),
...
def plot_boxplots(df: pd.DataFrame, out_dir: Path) -> None:
    ...
    df_sub["strategy"] = pd.Categorical(
        df_sub["strategy"], categories=ORDER, ordered=True
    )
    ...
    for alpha in BOXPLOT_ALPHAS:
        ...
        fig.suptitle(
            f"Distribuição (α = {alpha:.2f}, n = {tsize_default:,})", fontsize=11
        )
        fig.tight_layout()
...
def plot_heatmaps(df_params: pd.DataFrame, out_dir: Path) -> None:
    for alpha in HEATMAP_ALPHAS:
        ...
        for col, label in HEATMAP_METRICS:
            pivot = (
                ...
                .reset_index()
                .pivot(
                    index="cluster_threshold", columns="block_fill_limit", values=col
                )
            )
            if pivot.empty:
                continue
    ...
    for alpha in HEATMAP_ALPHAS:
        ...
        if chunk.empty:
            continue
        grp = chunk.groupby(["cluster_threshold", "block_fill_limit"])[
            "avg_insert_probes"
        ].mean()
        best_idx = grp.idxmin()
        best_rows.append(
...
def main() -> None:
    ...
    if not main_path.exists():
        ...
        agg.to_csv(
            out_dir.parent / "benchmark_summary.csv", index=False, float_format="%.6f"
        )
